...
Vy = 0; Vz = 0; Vw = 0;
% for i = 2:4
%     Vy = Vy + cos(pi*i)*(y - equil_y)^i/(i*equil_y^(i-1));
%     Vz = Vz + cos(pi*i)*(z - equil_z)^i/(i*equil_z^(i-1));
%     Vw = Vw + cos(pi*i)*(w - equil_w)^i/(i*equil_w^(i-1));
% end
for i = 1:4
    Vy = Vy - cos(pi*i)*(y - 0.5)^i/(i*0.5^(i));
    Vz = Vz - cos(pi*i)*(z - 0.5)^i/(i*0.5^(i));
    Vw = Vw - cos(pi*i)*(w - 0.5)^i/(i*0.5^(i));
end
...
V = y - equil_y*(log(equil_y) - expand(Vy)) + a1*(z - equil_z*(log(equil_z) - expand(Vz))) + a2*(w - equil_w*(log(equil_w) - expand(Vw))) + a3*w*y + a4*w*z + a5*y*z;
V = expand(V);
...
[prog,p1] = sospolyvar(prog,monomials(vars,0:orderV/2),'wscoeff');
[prog,p2] = sospolyvar(prog,monomials(vars,0:orderV/2),'wscoeff');
[prog,p3] = sospolyvar(prog,monomials(vars,0:orderV/2),'wscoeff');
[prog,p11] = sospolyvar(prog,monomials(vars,0:orderV/2),'wscoeff');
[prog,p22] = sospolyvar(prog,monomials(vars,0:orderV/2),'wscoeff');
[prog,p33] = sospolyvar(prog,monomials(vars,0:orderV/2),'wscoeff');
D1 = w - 0;
D2 = y - 0;
D3 = z - 0;
D11 = 1-w;
...
%derV = subs(derV,[w,y,z],[(w)^2-1,(y)^2-1,(z)^2-1]);
...
prog = sosineq(prog,derV - (p1*D1 + p2*D2 + p3*D3 + p11*D11 + p22*D22 + p33*D33));
